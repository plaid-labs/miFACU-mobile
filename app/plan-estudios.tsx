  ...
  useFocusEffect(
    React.useCallback(() => {
      const loadData = async () => {
        try {
          console.log('Cargando materias desde API...');
          const data = await api.getMaterias();
          console.log('Datos recibidos de API:', data);

          // Mapear datos de API al formato esperado por la app
          const mappedData = (data || []).map((materia: MateriaAPI): Materia => {
            // Convertir nivel romano a número
            const nivelRomano = materia.nivel?.toUpperCase() || 'I';
            let nivelNumero = 1;
            switch (nivelRomano) {
              case 'II': nivelNumero = 2; break;
              case 'III': nivelNumero = 3; break;
              case 'IV': nivelNumero = 4; break;
              case 'V': nivelNumero = 5; break;
              default: nivelNumero = 1;
            }
            ...
            return {
              ...
              nivel: nivelNumero,
              estado: 'pendiente', // Estado por defecto ya que la API no lo proporciona
              dia: undefined, // La API no proporciona horario
              hora: undefined,
              duracion: undefined,
              aula: undefined
            };
          ...
          try {
            ...
            console.log('Usando datos locales:', todas);
            setMaterias(todas || []);
          } catch (fallbackError) {
            ...
            <View style={styles.cardContent}>
              ...
              <View style={styles.infoRow}>
                <Text style={[styles.materiaInfo, { color: getColor(materia.estado) }]}>
                  {materia.estado ? materia.estado.toUpperCase() : 'SIN ESTADO'}
                </Text>
