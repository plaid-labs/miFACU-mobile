...
export default function DetalleMateriaScreen() {
  ...
  useEffect(() => {
    const loadMateria = async () => {
      if (id) {
        try {
          const data = await api.getMateriaById(id);
          if (data) {
            setMateria(data);
          ...
          try {
            const { getMateriaById } = require('../src/data/db');
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      // Fallback a datos locales
      try {
        const { updateMateria } = require('../src/data/db');
        updateMateria(id, {
          dia: nuevoDia.toUpperCase(),
          hora: horaNum,
          aula: nuevaAula
        ...
        setMateria({ ...materia, dia: nuevoDia.toUpperCase(), hora: horaNum, aula: nuevaAula });
        setEditMode(false);
        Alert.alert("¡Horario Actualizado!", "Guardado localmente.");
      } catch (fallbackError) {
